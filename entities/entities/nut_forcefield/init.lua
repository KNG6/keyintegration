...
function PLUGIN:haveLevl4(character)
...
end, "Seulement avec un CID valide."}
modes[2] = {function(client)
	local character = client:getChar()
	if (character:getInv():hasItem("cid4"))then
		return false
	else
		return true
	end
end, "N'accepter aucun citoyen."}
modes[3] = {function(client)
	local character = client:getChar()
	if (character:getInv():hasItem("cid2") or character:getInv():hasItem("cid3") or character:getInv():hasItem("cid4"))then
		return false
	else
		return true
	end
end, "Seulement level 2."}
modes[4] = {function(client)
	local character = client:getChar()
	if (character:getInv():hasItem("cid3") or character:getInv():hasItem("cid4"))then
		return false
	else
		return true
	end
end, "Seulement level 3."}
modes[5] = {function(client)
	return false
